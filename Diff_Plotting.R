...
fabianFol = "/Users/sophiewagner/Dropbox/HESS_Paper/Fabian/"

data <- read.table(file=paste0(fabianFol,"diff_pred_ll.txt"),header = TRUE,sep =",")

data <- data$diff_pred

diff <- read.table(file=paste0(fabianFol,"diff_dgg.txt"),header = TRUE,sep =",")


### Path
load("/media/Storage/paper_plots_Sophie/Diff_Data.RData")
resultFol = "/media/Storage/paper_plots_Sophie/"

test_data$diff <- as.numeric(test_data$diff)

range <- seq(-0.5,0.5,0.05)
mycolors <- colorRampPalette(rev(brewer.pal(11, "Spectral")))(length(range)-1)

png(paste0(fabianFol,"Diff_DGG.png"),width = 2000, height = 1000, units="px",res=400,pointsize = 4)
ggplot(data = grid) +
...
palette=mycolors
legendtitle = ""
png(paste0(fabianFol,"Diff_LL.png"), width = 2000, height = 1000, units = "px", res = 400, pointsize = 4)
